...
## BLOCKER CATALOG: RDS/Aurora Hidden Resilience Constraints (57 Blockers, 7 Categories)
...
| FT-06 | JVM default DNS TTL can be INFINITE (until JVM restart) if security manager is installed | [JVM TTL Settings](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.MultiAZ.Failover.html) | Java apps never reconnect without explicit TTL config |
| FT-07 | Aurora single-writer cluster without readers: NO automatic failover target exists | [Aurora Fast Failover](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraPostgreSQL.BestPractices.FastFailover.html) | Must launch new instance from scratch (10-15 min) |
| FT-08 | Aurora secondary cluster readers restart when primary writer restarts or fails over | [Aurora Global Database Limitations](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-global-database.html) | Global database secondary becomes unavailable during primary events |
| FT-09 | Single-AZ RDS instance: AZ failure = full outage requiring snapshot restore | Architectural constraint | RTO becomes snapshot restore time (30-60+ minutes) |
...
| SR-05 | Default parameter group assigned on restore — custom parameters LOST | [Parameter Group Considerations](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_RestoreFromSnapshot.html) | Performance tuning, replication settings, memory config all revert to defaults |
...
| SR-09 | Cannot restore from a shared AND encrypted snapshot directly | [Restoring from Snapshot](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_RestoreFromSnapshot.html) | Must copy snapshot first, then restore — adds time |
